...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const heroImgContainter = document.querySelector(".hero-img-container");
  ...
  const horizantScaleRatio = logoDimensions.width / logoBoundingBox.width;
  const verticalScaleRatio = logoDimensions.height / logoBoundingBox.height;
  const logoScaleFactor = Math.max(horizantScaleRatio, verticalScaleRatio);
  ...
  logoMask.setAttribute(
    "transform",
    `translate(${logoHorizontalPosition + offsetX},
               ${logoVerticalPosition + offsetY})
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (scrollProgress < 0.85) {
        ...
        const heroImgContainterScale = 1.5 - 0.5 * normalizedProgress;
        const overlayScale =
          initialOverlayScale *
          Math.pow(1 / initialOverlayScale, normalizedProgress);
        let fadeOverlayOpacity = 0;

        gsap.set(heroImgContainter, {
          scale: heroImgContainterScale,
        });
